...
func execHook(spec HookSpec) nacelle.Hook {
	return func(ctx context.Context, ev nacelle.HookEvent) nacelle.HookResult {
		...
		out, errOut, runErr := runCommand(ctx, spec.Run, hookPayload{
			Event: string(ev.Point), Tool: ev.Tool,
			Input: ev.Input, Result: ev.Result, Retry: ev.Retry,
		})
		return interpret(spec.Run, ev, runErr, out, errOut)
	}
}
...
// interpret turns one finished hook process into a decision. Injection only
// lands on AfterToolCall — there is no result to amend before the call —
// and is skipped on a failed tool: prose after an error reads to the model
// as if the error were handled when nothing was.
