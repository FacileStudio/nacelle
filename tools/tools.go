...
type Config struct {
	...
	// PathIsolation rejects paths that resolve outside Root.
	//
	...
	//
	// PATH is the one exception, and deliberately so: it names executables,
	// not credentials, and dropping it strands every CLI the person running
	// this installed outside /usr/bin — mise shims, ~/.local/bin, go
	// installs. The default PATH is therefore the one this process was
	// started with, which is the launching shell's own, with the standard
	// user bin directories appended in case that shell was itself minimal.
	CommandEnv []string
