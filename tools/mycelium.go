...
// Mycelium builds tools that give the model access to this machine's mycelium
...
// package adds.
//
// Absent mycelium is not an error. Most consumers of this package will not
...
func listFlowsTool() (nacelle.Tool, error) {
	return nacelle.NewTool("list_flows",
		"List the mycelium flows recorded on this machine, with what each one does and whether it is trusted to run yet. Call this before run_flow if you do not already know the exact flow name.",
		func(ctx context.Context, _ struct{}) (string, error) {
			out, err := mycelium(ctx, "flow", "list")
...
func runFlowTool() (nacelle.Tool, error) {
	return nacelle.NewTool("run_flow",
		"Run a named mycelium flow: a recorded, deterministic procedure, in place of an improvised shell pipeline. A flow nobody has reviewed on this machine yet refuses to run and says so — that is mycelium's own trust gate, not something to work around; tell the person running you if you hit it, since only a human can run `mycelium flow trust <name>` to clear it.",
		func(ctx context.Context, in runFlowInput) (string, error) {
			if strings.TrimSpace(in.Name) == "" {
				return "", fmt.Errorf("no flow name given")
			}
			out, err := mycelium(ctx, "flow", "run", in.Name)
			return report(out, err, DefaultMaxOutputBytes), nil
		})
}
...
func searchMemoryTool() (nacelle.Tool, error) {
	return nacelle.NewTool("search_memory",
		"Search mycelium's memory wiki — durable, project-specific notes an earlier session recorded: known bugs, tool gotchas, conventions, past decisions. Best match first. Prefer this over guessing when a project might already have a documented answer.",
		func(ctx context.Context, in searchMemoryInput) (string, error) {
			if strings.TrimSpace(in.Query) == "" {
				return "", fmt.Errorf("no query given")
			}
			out, err := mycelium(ctx, "memory", "search", in.Query, "--limit", "10")
			return report(out, err, DefaultMaxOutputBytes), nil
		})
}

// mycelium runs one mycelium subcommand and returns its combined output.
//
...
func mycelium(ctx context.Context, args ...string) (string, error) {
	cmd := exec.CommandContext(ctx, "mycelium", args...)
